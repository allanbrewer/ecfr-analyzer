...
def historical():
    ...
    if corrections_over_time_data:
        ...
        avg_per_year = round(total_corrections / len(corrections_list)) if corrections_list else 0
    ...
    for agency in hierarchy.get("agencies", []):
        ...
        if query in agency_name:
            ...
            if word_count_data and "agencies" in word_count_data:
                agency_word_count = word_count_data.get("agencies", {}).get(agency_slug, {})
                word_count = agency_word_count.get("word_count", 0)
            
            # Get corrections data
            corrections_count = 0
            if corrections_data and "agencies" in corrections_data:
                agency_corrections = corrections_data.get("agencies", {}).get(agency_slug, {})
        ...
        for child in agency.get("children", []):
            ...
            if query in child_name:
                ...
                word_count = 0
                if word_count_data and "agencies" in word_count_data:
                    child_word_count = word_count_data.get("agencies", {}).get(child_slug, {})
                    word_count = child_word_count.get("word_count", 0)
                
                # Get corrections data
                corrections_count = 0
                if corrections_data and "agencies" in corrections_data:
                    child_corrections = corrections_data.get("agencies", {}).get(child_slug, {})
...
def agency_detail():
    ...
    agency_word_count = word_count_data.get("agencies", {}).get(agency_slug, {}) if word_count_data else {}
    agency_corrections = corrections_data.get("agencies", {}).get(agency_slug, {}) if corrections_data else {}
    ...
    if is_parent:
        for child in agency_data.get("children", []):
            child_slug = child.get("slug")
            child_word_count = word_count_data.get("agencies", {}).get(child_slug, {}) if word_count_data else {}
            child_corrections = corrections_data.get("agencies", {}).get(child_slug, {}) if corrections_data else {}
